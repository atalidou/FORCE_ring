%% This code generates FIGURE 6 Rossler.

clear;
close all;

Q1_plus = -0.4;     % ring component (fwd)
Q1_minus = -0.1;    % ring component (bwd)
...
N = 3000;           % number of neurons in the network
...
icrit = round(40/dt);       % end simulation at this time step
...
b = 2;         
c = 4;          
TC = 10; % alter the time scale 

[t,y] = ode45(@(t,y) rossler(TC, a, b, c, t, y) , [0, T], [1; 1; 1]); 
zx(:,1) = interp1(t,y(:,1),tx);
zx(:,2) = interp1(t,y(:,2),tx);
zx(:,3) = interp1(t,y(:,3),tx);
...
for j = 1:N
    ...
    % index of forward neighbor
    idx_fwd = mod(j, N) + 1;

    % index of backward neighbor
    idx_bwd = mod(j-2, N) + 1;

    % weights
    OMEGA(idx_fwd, j) = OMEGA(idx_fwd, j) + Q1_plus;
    OMEGA(idx_bwd, j) = OMEGA(idx_bwd, j) + Q1_minus;
...
for i = 1:nt
    ...
    if mod(i, round(0.5/dt)) == 1
        ...
        figure(4)
        drawnow 
        plot(dt*(1:i), zx(1:i,3), 'k--', 'LineWidth', 2)
        hold on
        plot(dt*(1:i), current(1:i,3), 'LineWidth', 2)
        hold off
        xlim([dt*i-5, dt*i])
...
function dy = rossler(TC, a, b, c, t, y)

    dy(1,1) = -y(2,1) - y(3,1); 
    dy(2,1) = y(1,1) + a*y(2,1);
    dy(3,1) = b + y(3,1)*(y(1,1) - c);
    dy = TC*dy;
